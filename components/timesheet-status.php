        ...
        <div class="mt-4 grid grid-cols-1 md:grid-cols-5 gap-4">
            <div class="bg-gradient-to-r from-primary-50 to-primary-100 rounded-lg p-4 border-l-4 border-primary-500 hover:shadow-md transition-all duration-300">
                ...
                <p class="text-xl font-bold mt-1 text-primary-800">
                    <?php 
  // Get seconds from the raw In and Out times instead of the pre-calculated column
  $ts_in = strtotime($current_timesheet['am_timein']);
  $ts_out = strtotime($current_timesheet['am_timeOut']);
  $cap = strtotime(date('Y-m-d', $ts_in) . ' 12:00:00');
  
  $seconds = max(0, min($ts_out, $cap) - $ts_in);
  
  if ($seconds >= 18000) {
      echo "5hr";
  } else {
      echo formatDuration($current_timesheet['am_hours_worked']);
  }
?>
                </p>
            </div>
            ...
            <div class="bg-gradient-to-r from-primary-50 to-primary-100 rounded-lg p-4 border-l-4 border-primary-500 hover:shadow-md transition-all duration-300">
                ...
                <p class="text-xl font-bold mt-1 text-primary-800">
    <?php 
    if(!isTimeEmpty($current_timesheet['pm_timein']) && !isTimeEmpty($current_timesheet['pm_timeout'])): 
        
        $row_date = date('Y-m-d', strtotime($current_timesheet['created_at']));
        $intern_id = $current_timesheet['intern_id']; 

        // 1. Fetch Custom Limit (The "Cast" feature)
        ...
        if ($custom_limit) {
            ...
            // 2. Fallback to System Rule (8hrs = 5pm, 9hrs+ = 6pm)
            ...
            // If required hours is 8, cap at 5pm (13 + 4). If 9, cap at 6pm (13 + 5).
            $pm_shift_length = ($required_hrs <= 8) ? 4 : 5; 
            $official_end_time = strtotime($row_date . ' ' . (13 + $pm_shift_length) . ':00:00');
        }

        // 3. Calculation
        $actual_in = strtotime($row_date . ' ' . $current_timesheet['pm_timein']);
        $actual_out = strtotime($row_date . ' ' . $current_timesheet['pm_timeout']);
        $official_start = strtotime($row_date . ' 13:00:00');

        // Apply Caps: Don't start before 1pm, don't end after Official End
        $start_counting = max($actual_in, $official_start);
        ...
        if ($pm_seconds > 0): 
            ...
            // Format to match your desired look: "4hr 30m" or "4hr"
            if ($h > 0 && $m > 0) {
                echo $h . "hr " . $m . "m";
            } elseif ($h > 0) {
                echo $h . "hr";
    ...
    ?>
        ...
        <div class="bg-gradient-to-r from-primary-100 to-primary-200 rounded-lg p-4 border-l-4 border-primary-600 hover:shadow-md transition-all duration-300">
                ...
                <p class="text-xl font-bold mt-1 text-primary-900">
                    <?php 
                    if(!isTimeEmpty($current_timesheet['day_total_hours'])) {
                        $total_val = $current_timesheet['day_total_hours'];
                        // Extracting only the hour number from the HH:MM:SS format
                        if (strpos($total_val, ':') !== false) {
                            $parts = explode(':', $total_val);
                            echo (int)$parts[0] . " hr";
                        } else {
                            echo (int)$total_val . " hr";
                        }
                    } else {
                        echo "-";
                    }
