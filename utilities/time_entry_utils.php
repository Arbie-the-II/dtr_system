    ...
    if ($check_stmt->rowCount() > 0) {
        ...
        if ($current_hour < 12) {
            ...
            if (isTimeEmpty($timesheet_data['am_timein'])) {
                ...
                if ($current_hour < 7 || ($current_hour == 7 && $current_minute == 0)) {
                    $calc_time = '07:00:00'; // Set to 8:00 AM for calculation
                } else {
            ...
            if (isTimeEmpty($timesheet_data['pm_timein'])) {
                // NEW RESTRICTION: Block entry before 12:30 PM
                if ($current_hour == 12 && $current_minute < 30) {
                    $result['message'] = "PM Time-in is allowed only from 12:30 PM onwards.";
                    $result['message_type'] = "error";
                    return $result;
                }

                // Check if morning time-out is recorded...
                if (!isTimeEmpty($timesheet_data['am_timein']) && isTimeEmpty($timesheet_data['am_timeOut'])) {
                    ...
                    // Apply the rounding rule: 12:30 PM to 1:00 PM becomes 1:00 PM
        ...
        if ($current_hour < 12) {
            ...
            // NEW RESTRICTION: Block entry before 12:30 PM
            if ($current_hour == 12 && $current_minute < 30) {
                $result['message'] = "Shortcut Denied: PM Time-in is allowed only from 12:30 PM onwards.";
                $result['message_type'] = "error";
                return $result;
            }
    ...
    if ($check_stmt->rowCount() > 0) {
        ...
        if (!isTimeEmpty($timesheet['am_timein']) && isTimeEmpty($timesheet['am_timeOut'])) {
            ...
            if ($has_overtime_started) {
                ...
                // 1. Calculate raw seconds from the source columns
                $raw_in = strtotime($timesheet['am_timein']); 
                $raw_out = strtotime($current_time);
                $cap_time = strtotime(date('Y-m-d', $raw_in) . ' 12:00:00'); // Forces the date to match

                // 2. THE FIX: Pick the smaller time (12:00 PM if it's currently 12:03 PM)
                $effective_out = min($raw_out, $cap_time);

                // 3. Calculate seconds and convert to HH:MM:SS
                $am_seconds = max(0, $effective_out - $raw_in);
                
                // Apply the 5-hour cap just in case
                if ($am_seconds > 18000) { $am_seconds = 18000; }

                $h = floor($am_seconds / 3600);
                $m = floor(($am_seconds % 3600) / 60);
                $s = $am_seconds % 60;
                
                // This variable now holds "05:00:00"
                $hours_worked = sprintf('%02d:%02d:%02d', $h, $m, $s);
                ...
                $update_stmt->bindParam(':time_out', $current_time); // Real clock time (12:03 PM)
                $update_stmt->bindParam(':hours_worked', $hours_worked); // Capped hours (05:00:00)
                $update_stmt->bindParam(':intern_id', $intern_id);
                $update_stmt->bindParam(':current_date', $current_date);
    if ($update_stmt->execute()) {
        updateTotalHours($conn, $intern_id, $current_date);
        ...
        $result['message'] = "Morning time-out recorded successfully at " . formatTime($current_time) . 
            ". Total hours (capped at 12:00 PM): " . formatDuration($hours_worked);
        $result['message_type'] = "success";
    } else {
        $result['message'] = "Error recording time out. Please try again.";
        $result['message_type'] = "error";
