...
if (isPostRequest()) {
    ...
    try {
        ...
        // 1. THIS RESTRICTION HERE
        $current_hour = (int)date('H');
        $current_minute = (int)date('i');

        if ($action === 'time_in' && $current_hour == 12 && $current_minute < 30) {
            // Clear any old pending photos from the session so they don't "add up"
            unset($_SESSION['pending_photo']);
            
            $response['message'] = 'PM Time-in is allowed only from 12:30 PM onwards.';
            header('Content-Type: application/json');
            echo json_encode($response);
            exit(); // kills the process BEFORE file_put_contents
        }
