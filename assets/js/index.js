...
/**
 * Formats time to show 'X hr Y min' unless it hits the limit,
 * in which case it strips the minutes entirely.
 * * @param {number} totalMinutes - Total calculated minutes
 * @param {number} limitHours - The admin-defined limit (e.g., 10)
 */
function formatCappedTotalHours(totalMinutes, limitHours) {
    const hours = Math.floor(totalMinutes / 60);
    const mins = totalMinutes % 60;

    // THE FIX: If hours meet or exceed the limit, ignore all minutes (1-59)
    if (hours >= limitHours) {
        return `${limitHours} hr`; 
    }

    // Otherwise, show normal formatting
    if (hours > 0 && mins > 0) {
