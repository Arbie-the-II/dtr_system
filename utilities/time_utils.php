...
function getLimitedHours($actual_hours, $conn) {
    // Fetch the flexible limit from DB
    $stmt = $conn->query("SELECT setting_value FROM system_settings WHERE setting_key = 'max_daily_hours'");
    $limit = (float)$stmt->fetchColumn() ?: 8.0;

    // The "30-minute grace" rule
    // If they work 10.6 hours and limit is 10, show only 10.0
    if ($actual_hours > ($limit + 0.5)) {
        return $limit;
    }
